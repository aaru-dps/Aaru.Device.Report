        ...
        while(user_response != 'N' && user_response != 'n')
        {
            ...
            if(user_response == 'Y' || user_response == 'y')
            {
                ...
                if(identify->Cylinders > 0 && identify->Heads > 0 && identify->SectorsPerTrack != 0)
                {
                    ...
                    blocks =
                            le16toh(identify->Cylinders) *
                            le16toh(identify->Heads) *
                            le16toh(identify->SectorsPerTrack);
                    xmlTextWriterEndElement(xmlWriter); // </CHS>
                }

                if(identify->CurrentCylinders > 0 &&
                   identify->CurrentHeads > 0 &&
                ...
                xmlTextWriterWriteFormatElement(xmlWriter,
                                                BAD_CAST "SupportsReadLba",
                                                "%s",
                                                (!error &&
                                                 (ata_error_lba->status & 0x01) != 0x01 &&
                                                 ata_error_lba->error == 0 &&
                                                 buffer != NULL) ? "true" : "false");

                printf("Trying READ SECTOR(S) RETRY in LBA mode...\n");
                error = ReadLba(fd, &buffer, &ata_error_lba, TRUE, 0, 1);
                xmlTextWriterWriteFormatElement(xmlWriter,
                                                BAD_CAST "SupportsReadRetryLba",
                                                "%s",
                                                (!error &&
                                                 (ata_error_lba->status & 0x01) != 0x01 &&
                                                 ata_error_lba->error == 0 &&
                ...
                xmlTextWriterWriteFormatElement(xmlWriter,
                                                BAD_CAST "SupportsReadDmaRetryLba",
                                                "%s",
                                                (!error &&
                                                 (ata_error_lba->status & 0x01) != 0x01 &&
                                                 ata_error_lba->error == 0 &&
                ...
                xmlTextWriterWriteFormatElement(xmlWriter,
                                                BAD_CAST "SupportsReadLba48",
                                                "%s",
                                                (!error &&
                                                 (ata_error_lba48->status & 0x01) != 0x01 &&
                                                 ata_error_lba48->error == 0 &&
                                                 buffer != NULL) ? "true" : "false");

                printf("Trying READ DMA in LBA48 mode...\n");
                error = ReadDmaLba48(fd, &buffer, &ata_error_lba48, 0, 1);
                xmlTextWriterWriteFormatElement(xmlWriter,
                                                BAD_CAST "SupportsReadDmaLba48",
                                                "%s",
                                                (!error &&
                                                 (ata_error_lba48->status & 0x01) != 0x01 &&
                                                 ata_error_lba48->error == 0 &&
                ...
                xmlTextWriterWriteFormatElement(xmlWriter,
                                                BAD_CAST "SupportsReadLongRetry",
                                                "%s",
                                                (!error &&
                                                 (ata_error_chs->status & 0x01) != 0x01 &&
                                                 ata_error_chs->error == 0 &&
                ...
                xmlTextWriterWriteFormatElement(xmlWriter,
                                                BAD_CAST "SupportsReadLongRetryLba",
                                                "%s",
                                                (!error &&
                                                 (ata_error_lba->status & 0x01) != 0x01 &&
                                                 ata_error_lba->error == 0 &&
